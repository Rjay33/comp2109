...
// Creates Shoe Reviews Custom Post type
// function shoe_reviews_init() - Here we create a new function for our custom post type.  We do this to limit conflicts with any other code inside our function.php file.  It is also a best practice when adding new code to any file like functions.php
function shoe_reviews_init(){
	$args = array(
		...
		'hierarchical' => false,
		// rewrite - This rule is either true or false.  The default is true so if slug argument is entered then the slug name is prepended to the posts.  Our slug "shoe-reviews" will be prepended to each new post of that type.
		'rewrite' => array('slug' => 'shoe-reviews'),
		...
		register_post_type( 'shoe-reviews', $args );
}
add_action( 'init', 'shoe_reviews_init' );

//  Lets make our first shortcode
function shoe_reviews(){
	$query = new WP_Query( array('post_type' => 'shoe-reviews', 'posts_per_page' => 10 ) );
	while ( $query->have_posts() ) : $query->the_post();
	echo "<div><h3>";
	the_title();
	echo "</h3></div>";
	echo "<div class='shoe-content'>";
		the_post_thumbnail();
		the_content();
	echo "</div>";
	endwhile;
}
function shoe_shortcode(){
	add_shortcode('shoe_shortcode', 'shoe_reviews');
}
add_action( 'init', 'shoe_shortcode');
